...
/**
 * A custom 2D chart control that displays data points in a 2D coordinate system. This control
 * allows visualization of datasets with X and Y dimensions.
 */
public class Chart2DController extends Pane {
  ...
  private static final Color[] SERIES_FILL_COLORS =
      new Color[] {
        Color.web("#3b82f6"), // azul
        Color.web("#10b981"), // verde
        Color.web("#ef4444") // rojo
      };
  private static final Color[] SERIES_BORDER_COLORS =
      new Color[] {
  ...
  public Chart2DController() {
    ...
    FXMLLoader fxmlLoader =
        new FXMLLoader(getClass().getResource("/com/davidpe/tasker/ui/controls/chart-2d.fxml"));
  ...
  /**
   * Establece múltiples datasets (hasta 2). Si se pasan más, se ignoran a partir del tercero. Cada
   * lista interna representa una serie, y cada índice i representa el mismo punto X.
   */
  ...
  private void drawAxes(GraphicsContext gc, double chartWidth, double chartHeight) {
    ...
    // X axis
    gc.strokeLine(
        MARGIN_LEFT, MARGIN_TOP + chartHeight, MARGIN_LEFT + chartWidth, MARGIN_TOP + chartHeight);
    // Y axis
  ...
  private void drawDataPoints(GraphicsContext gc, double chartWidth, double chartHeight) {
    ...
    if (!datasets.isEmpty()) {
      ...
      double barWidth =
          (numSeries > 0) ? (barsTotalWidth / numSeries) : 0.0; // ancho por serie dentro del grupo
      ...
      for (int i = 0; i < maxPoints; i++) {
        ...
        for (int s = 0; s < numSeries; s++) {
          ...
          Stop[] stops =
              new Stop[] {new Stop(0, SERIES_BORDER_COLORS[s]), new Stop(1, SERIES_FILL_COLORS[s])};
      ...
      if (!xAxisLabels.isEmpty()) {
        ...
        for (int i = 0; i < maxPoints; i += step) {
          ...
          double groupCenterX =
              MARGIN_LEFT
                  + groupSpacing
                  + (i * (totalBarGroupWidth + groupSpacing))
                  + (totalBarGroupWidth / 2);
      ...
      if (maxXLabels > 0) {
        ...
        for (int i = 0; i <= maxXLabels; i++) {
          ...
          int index =
              Math.min(
                  dataset.size() - 1,
                  Math.max(0, (int) Math.round((double) i * (dataset.size() - 1) / maxXLabels)));
          double x = MARGIN_LEFT + barSpacing + (index * (barWidth + barSpacing)) + (barWidth / 2);
          double value = minX + ((maxX - minX) * i / maxXLabels);
          String label;
          if (epochMillis || epochSeconds) {
            long epochMs =
                (epochMillis) ? (long) Math.round(value) : (long) Math.round(value * 1000.0);
  ...
  private void drawLegend(
    ...
    for (int i = 0; i < numSeries; i++) {
      ...
      labels[i] =
          (i < seriesNames.size() && seriesNames.get(i) != null && !seriesNames.get(i).isEmpty())
              ? seriesNames.get(i)
              : fallback;
    }
    ...
    double startX =
        MARGIN_LEFT
            + chartWidth
            - totalLegendWidth; // alineado a derecha dentro del área bajo eje X
  ...
  private void playShowAnimation() {
    ...
    showTimeline =
        new Timeline(
            ...
            new KeyFrame(
                Duration.millis(650), new KeyValue(animationProgress, 1.0, Interpolator.EASE_OUT)));
  ...
  /**
   * Intenta parsear etiquetas en formato dd/MM, dd/MM/yy o dd/MM/yyyy y devolver "dd/MMM" (ej.
   * 01/Sep). Si no se puede parsear, devuelve el label original.
   */
